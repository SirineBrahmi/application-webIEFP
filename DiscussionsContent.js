...
const DiscussionsContent = () => {
  const { userId: currentUserId, userPrenom, userNom, userRole, loading: contextLoading, error: contextError } = useOutletContext();
...
useEffect(() => {
    if (!currentUserId) {
      setError('Identifiant utilisateur non disponible. Veuillez vous reconnecter.');
      setLoading(false);
      return;
    }
      ...
      try {
        ...
        if (formateursSnapshot.exists()) {
          formateursSnapshot.forEach(formateur => {
            ...
            if (!formateurIds.has(formateurData.uid)) {
              contactsList.push({
        ...
        if (etudiantsSnapshot.exists()) {
          etudiantsSnapshot.forEach(etudiant => {
            const etudiantData = etudiant.val();
            // Exclure l'utilisateur courant de la liste
            if (etudiantData.uid !== currentUserId && !studentIds.has(etudiantData.uid)) {
              contactsList.push({
                id: etudiantData.uid,
              ...
              studentIds.add(etudiantData.uid);
            }
          });
        }

        // 4. Récupérer les groupes (si nécessaire)
            ...
            <>
              ...
              <MessagesArea>
                {messages.map(message => (
                  <Message key={message.id} isFormateur={message.sender === currentUserId}>
                    <MessageBubble isFormateur={message.sender === currentUserId}>
                      {message.text}
                    </MessageBubble>
